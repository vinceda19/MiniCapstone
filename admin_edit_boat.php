...
// Check if the boat ID is passed as a parameter
if (!isset($_GET['id']) || empty($_GET['id'])) {
    header("Location: admin_dashboard.php"); // Redirect to the admin dashboard if no boat ID
    exit();
}
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $stmt = $conn->prepare("UPDATE boats SET boat_name = ?, capacity = ?, description = ?, image_path = ? WHERE id = ?");
    $stmt->bind_param("sissi", $boat_name, $capacity, $description, $image_path, $boat_id);
...
<body class="bg-gray-100 font-sans">
    ...
    <div class="max-w-4xl mx-auto my-8 p-6 bg-white rounded-lg shadow-lg">
        ...
        <form action="admin_edit_boat.php?id=<?php echo $boat['id']; ?>" method="POST" enctype="multipart/form-data">
            <!-- Boat Name -->
            <div class="mb-4">
        ...
        <div class="mt-4 text-center">
            <a href="admin_dashboard.php" class="text-gray-600 hover:underline">Back to Dashboard</a>
        </div>
    </div>
